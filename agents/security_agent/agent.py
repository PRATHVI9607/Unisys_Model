...
class EntropyCalculator:
    ...
    @staticmethod
    def calculate_file_entropy_random(file_paths: List[str], max_samples: int = 20) -> float:
                        ...
                        if "kubepods" in cgroup:
                            ...
                            for part in parts:
                                if part.startswith("pod"):
                                    pod_name = part.split("-", 1)[1] if "-" in part else part
...
class SecurityAgent:
    ...
    async def _direct_kill(self, pid: int, risk_score: float, early_signals: Dict) -> None:
