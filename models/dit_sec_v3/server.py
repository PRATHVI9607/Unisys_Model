...
async def explain(request: ScoreRequest):
    ...
    if request.new_spec and request.old_spec:
        explainability["yaml_fields"] = _explain_yaml_diff(request.old_spec, request.new_spec)
...
def _explain_metrics(metrics: List[List[float]]) -> Dict:
    ...
    if metrics and len(metrics) > 0:
        contributions["cpu_throttle"] = min(1.0, float(metrics[0][0])) if len(metrics[0]) > 0 else 0.0
        contributions["memory_usage"] = min(1.0, float(metrics[0][1])) if len(metrics[0]) > 1 else 0.0
